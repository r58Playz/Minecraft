...
class Type:
    ...
    def read(cls, file_object):
        if cls.read_with_context == Type.read_with_context:
            raise NotImplementedError('One of "read" or "read_with_context" must be overridden in a subclass.')
        else:
            raise TypeError('This type requires a ConnectionContext: call "read_with_context" instead of "read".')

    @classmethod
    def send(cls, value, socket):
        if cls.send_with_context == Type.send_with_context:
            raise NotImplementedError('One of "send" or "send_with_context" must be overridden in a subclass.')
        else:
            raise TypeError('This type requires a ConnectionContext: call "send_with_context" instead of "send".')
        
class Boolean(Type):
    @staticmethod
    def read(file_object):
        return struct.unpack('?', file_object.read(1))[0]
    def write(value, socket):
